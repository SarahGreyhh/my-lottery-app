...
function Header() {
  ...
  return (
    <header className='grid grid-cols-2 md:grid-cols-5 justify-between items-center p-5 '>
        ...
        <div>
         <h1 className='text-lg text-white font-bold'> SARAH LOTTERY DRAW</h1>
         <p className='text-xs text-emerald-300  truncate'> User: {address?.substring (0,5)}...{address?.substring(address.length, address.length -5)} </p>
         </div>
       </div>

    <div className=' hidden md:flex  md:col-span-3
     items-center justify-center rounded-md' >
        <div className='bg-[#3ca7ff] p-4 space-x-2 ' >
